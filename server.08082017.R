...
shinyServer(function(input, output) {
  dataInput <- reactive({
    ...
    data.date="01212017"
    table.2.01212017=read.csv(infile, sep="\t", header=FALSE, stringsAsFactors=FALSE)
  ...
  if(input$type=="density"){
    if(input$analysis!="53UTRCDS"){
    plot(density(dataInput(), bw=50), col=input$color,xlim = c(input$range) , main = paste(input$org,input$analysis,"density plot",sep = " "),xlab =paste("the distance in base pairs between the peak and the\n",input$analysis,
                                                                                                                                                         "(negative for upstream peak and positive for downstream peak)",sep = " "))
      lines(density(dataInput2(),bw=50),col=input$color2,xlim=c(input$range)) 
    }
    else{
      if(input$org=="hg19"){
        mean.utr.3.size.allgenes=1363.202
        mean.utr.5.size.allgenes=254.5384
        mean.cds.size.allgenes=1638.583
      plot(density(dataInput(), bw=50), xaxt="n",col=input$color,xlim = c(0,3200) , main = paste(input$org,input$analysis,"density plot",sep = " "))
      lines(density(dataInput2(),bw=50),col=input$color2,xlim=c(0,3200))
      axis(1, c(mean.utr.5.size.allgenes, mean.utr.5.size.allgenes+mean.cds.size.allgenes, mean.utr.5.size.allgenes+mean.cds.size.allgenes+mean.utr.3.size.allgenes),
        ...
        mean.utr.5.size.allgenes=218.4317
        mean.cds.size.allgenes=1572.418
        plot(density(dataInput(), bw=50), xaxt="n",col=input$color,xlim = c(0,3200) , main = paste(input$org,input$analysis,"density plot",sep = " "),xlab = "The normalized distribtion of the peaks in any given gene \n (5'UTR, CDS and 3'UTR in each gene are normalized to be the averages of their values using all genes)")
        lines(density(dataInput2(),bw=50),col=input$color2,xlim=c(0,3200))
        axis(1, c(mean.utr.5.size.allgenes, mean.utr.5.size.allgenes+mean.cds.size.allgenes, mean.utr.5.size.allgenes+mean.cds.size.allgenes+mean.utr.3.size.allgenes),
             labels=c("5-UTR", "CDS", "3-UTR"))
      }
    ...
    if(input$analysis!="53UTRCDS"){
    hist(dataInput(),col=input$color,xlim = c(input$range) , main = paste(input$org,input$analysis,"density plot",sep = " "))
    }
    else{
      hist(dataInput(), breaks=20, prob=TRUE, col=input$color)
    }
  }
  })
